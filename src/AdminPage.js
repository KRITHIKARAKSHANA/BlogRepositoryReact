  ...
  if (!flag) {
    return (
         ...
         <a href="/" className="d-flex align-items-center pb-0 mb-md-0 me-md-auto px-3" style={{color:"#5D534A"}}>
                  ...
                  <div className="dropdown pe-4 ">
                        ...
                        if ((user.profilePic) == null) {
                            ...
                            return (
                                get.filter(file => file.url.substring("http://localhost:8085/files/".length) == user.profilePic.id).map((a) => (
                                    <img width="30" height="30" className="rounded-circle" src={a.url} alt="img" />
                                ))
                            )
                        }
                    })()}
                      ...
                      <ul className="dropdown-menu dropdown-menu-dark text-small shadow pe-0" style={{minWidth:60+"px",backgroundColor:"#5D534A"}} aria-labelledby="dropdownUser1">
                          ...
                          <li>
                            <button onClick={event=>props.logout1("any")} className='btn btn-danger w-100 text-decoration-none text-light'>
      ...
      <div className="row flex-nowrap">
          <div className="col-auto col-md-3 col-xl-2 px-sm-2 px-0 " style={{backgroundColor :"#E5D3B7"}}>
              <div className="d-flex flex-column align-items-center align-items-sm-start px-3 pt-2 text-white min-vh-100">
                  ...
                  <ul className="nav flex-column mb-sm-auto mb-0 align-items-center align-items-sm-start" id="menu">
                      ...
                      <li className="nav-item nav-link align-middle px-0" style={{color:"#5D534A"}}>
                      <i className="bi fs-2 bi-envelope-paper"></i> <span className="ms-1 d-none d-sm-inline" ><Link to="viewRequest" style={{color:"#5D534A",fontFamily:"Fantasy,Copperplate",fontSize:25+"px"}}>VIEW REQUEST</Link></span>
                         
                      </li>
                     
                      <li className="nav-item nav-link align-middle px-0" style={{color:"#5D534A",fontFamily:"Fantasy,Copperplate",fontSize:25+"px"}}>
                      <i className="bi fs-2 bi-people"></i> <span className="ms-1 d-none d-sm-inline" ><Link to="viewUser" style={{color:"#5D534A",fontFamily:"Fantasy,Copperplate",fontSize:25+"px"}}>VIEW USER</Link></span> 
                      </li>
                      <li className="nav-item nav-link align-middle px-0" style={{color:"#5D534A"}}>
                      <i className="bi fs-2 bi-bookmark-plus"></i> <span className="ms-1 d-none d-sm-inline" ><Link to="addCat" style={{color:"#5D534A",fontFamily:"Fantasy,Copperplate",fontSize:25+"px"}}>ADD CATEGORY</Link></span>
          ...
          <Routes>
                    <Route path="home" element={<AHomePage showNext={changePage}/>}></Route>
                    <Route path="viewRequest" element={<ViewRequest/>}></Route>
                    <Route path="viewUser" element={<ViewUser/>}></Route>
                    <Route path="viewBlog" element={<ViewBlog user1={props.u} bid={blogid} goBack={backPage} />}></Route>
